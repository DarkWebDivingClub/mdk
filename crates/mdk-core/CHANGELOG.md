...
- `self_update` now converges the member's local wire-format policy to `MIXED_CIPHERTEXT_WIRE_FORMAT_POLICY` alongside the leaf-capabilities refresh shipped in [#261](https://github.com/marmot-protocol/mdk/pull/261). No-op when the policy is already `MIXED_CIPHERTEXT`. A persist failure during convergence is logged at `warn` and does not fail the rotation — the in-memory config remains correct for the session and the next self_update retries. Reduces the population of members that would need a wire-format reconciliation at a future `GroupContextExtensions` upgrade event. Observable only to callers that read `group.configuration().wire_format_policy()` directly. Follow-up to [#261](https://github.com/marmot-protocol/mdk/pull/261). ([#264](https://github.com/marmot-protocol/mdk/pull/264))
...
- **GroupContextExtensions upgrade path.** Three new public methods on `MDK` let client UX prompt an admin to upgrade a mixed group that has since become all-modern:
  ...
  - `upgrade_group_capabilities(&GroupId, proposals_to_add: &BTreeSet<ProposalType>) -> Result<UpdateGroupResult, Error>` — admin-only. Proposes a `GroupContextExtensions` commit adding the caller-selected proposal types to `RequiredCapabilities`. Per-proposal re-validation when the method runs surfaces TOCTOU via `Error::ProposalAlreadyRequired` (another admin upgraded concurrently) or `Error::ProposalNotAvailableForUpgrade { proposal, blockers }` (a new blocker appeared); the caller-bug cases surface as `Error::EmptyUpgradeSet`, `Error::ProposalNotInSupportedSet(pt)`, or `Error::NotAdmin`. All-or-nothing over the requested set — no partial commit is ever sent. Additive-only; clients drive the UX, MDK does not upgrade groups automatically. Converges the upgrader's local wire-format policy to `MIXED_CIPHERTEXT_WIRE_FORMAT_POLICY` alongside the commit, mirroring `self_update`. New types exported via `mdk_core::prelude`: `MemberCapabilities`, `CapabilityUpgradeStatus`, `ProposalUpgradability`, plus `ExtensionType` and `VerifiableCiphersuite` re-exports from OpenMLS. ([#266](https://github.com/marmot-protocol/mdk/pull/266))
- Added `MDK::group_required_proposals(&GroupId) -> Result<BTreeSet<ProposalType>, Error>`, returning the proposal types required of every member of a group from the live `MlsGroup`'s `RequiredCapabilities` extension. Lets consumers branch UI on per-group capability state (e.g. whether `SelfRemove` is available) without duplicating the extension walk MDK already performs internally. Returns `Error::GroupNotFound` if the MLS record is missing — an empty set unambiguously means "no required proposals" (the LCD outcome for mixed or empty-invitee groups). ([#265](https://github.com/marmot-protocol/mdk/pull/265))
...
- `create_group` no longer persists the creator's `SignatureKeyPair` before every invitee KeyPackage has parsed successfully. A malformed invitee KeyPackage now fails the call cleanly with no orphan signer left in provider storage. Follow-up to [#261](https://github.com/marmot-protocol/mdk/pull/261). ([#262](https://github.com/marmot-protocol/mdk/pull/262))
